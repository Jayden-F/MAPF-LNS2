...
public:
	...
	AStarNode(int loc, int g_val, int h_val, LLNode *parent, int timestep, int num_of_conflicts) : LLNode(loc, g_val, h_val, parent, timestep, num_of_conflicts) {}
...
public:
	...
	Path findPath(const ConstraintTable &constraint_table, int planning_window_length = MAX_TIMESTEP);
	...
	SpaceTimeAStar(const Instance &instance, int agent) : SingleAgentSolver(instance, agent) {}
