...
class cchunk {
  public:
    ...
    inline char *first_addr() { return mem_; }

    inline size_t pool_size() { return pool_size_; }
...
class cpool {
  public:
    ...
    void print(std::ostream &out) {
        out << "warthog::mem::cpool #chunks: " << num_chunks_ << " #max_chunks "
  ...
  private:
    ...
    cpool &operator=(const cpool &other) { return *this; }
...
class MemoryPool {
  ...
  public:
    ...
    MemoryPool(uint64_t size) { init(size); };
    ...
    bool has_node(u_int64_t id) {
        // if (id >= numblocks_)
        // {
        //     std::cout << "range out of memory pool size " << id << "," <<
        //     numblocks_ << std::endl; exit(1);
        // }
    ...
    SIPPNode *generate(u_int64_t node_id) {
        ...
        if (!blocks_[block_id]) {
            ...
            blocks_[block_id] =
                new (blockspool_->allocate()) SIPPNode[node_pool_ns::NBS];
    ...
    SIPPNode *generate_node(u_int64_t id, int location, int g_val, int h_val,
                            SIPPNode *parent, int timestep, iterator interval) {
        // if (id >= numblocks_)
        // {
        //     std::cout << "range out of memory pool size " << id << "," <<
        //     numblocks_ << std::endl; exit(1);
        ...
        SIPPNode *node = generate(id);
        if (node->label == label_ && node->id != -1) {
            std::cout << "node already generated " << id << "," << is_ready()
                      << std::endl;
        ...
        if (node->location != location && node->location != -1) {
            std::cout << "node location is different " << node->location << ","
                      << location << std::endl;
            exit(1);
        }
    ...
    size_t mem() {
        size_t bytes =
            sizeof(*this) + blockspool_->mem() + numblocks_ * sizeof(void *);
