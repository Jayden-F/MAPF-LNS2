...
struct Interval : tuple<int, int, bool>
{
    using tuple::tuple;
    constexpr Interval(int a, int b, bool c) noexcept : tuple(a, b, c)
    {
    }
    constexpr Interval(tuple<int, int, bool> t) noexcept : tuple(t)
    {
    }
    using tuple::operator=;
}; // [t_min, t_max), has collision

struct Collision_Interval : tuple<int, int, int, bool, bool>
{
    using tuple::tuple;
    constexpr Collision_Interval(int a, int b, int c, bool d, bool e) noexcept : tuple(a, b, c, d, e)
    {
    }
    using tuple::operator=;
};
...
public:
    ...
    ReservationTable(const ConstraintTable &constraint_table, int goal_location) : constraint_table(constraint_table), goal_location(goal_location), sit(constraint_table.map_size) {}
