...
template <class Comparator = LLNode::compare_node,
          class QType = min_q>
class pqueue
{
public:
    ...
    mem()
    {
        return maxsize_ * sizeof(SIPPNode *) + sizeof(*this);
    }
...
private:
    ...
    resize(unsigned int newsize)
    {
        ...
        SIPPNode **tmp = new SIPPNode *[newsize];
        for (unsigned int i = 0; i < queuesize_; i++)
        {
            tmp[i] = elts_[i];
        }
...
typedef pqueue<LLNode::compare_node, min_q> pqueue_min;
