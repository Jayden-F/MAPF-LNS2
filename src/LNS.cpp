    ...
    while (num_agents_at_goal < max_agents && ((fsec)(Time::now() - time)).count() < T)
    {
        ...
        while (p != shuffled_agents.end())
        {
            ...
            if (agents[id].path.empty())
            {
                ...
                {
                    int r_id = *rp;
                    sipp_intervals.remove_path(agents[r_id].id, agents[r_id].path, current_timestep, MAX_TIMESTEP, true);
                    ++rp;
                }
                if (screen >= 2)
        ...
        if (remaining_agents == 0)
        {
            ...
            for (int id = 0; id < max_agents; id++)
            {
                ...
                sipp_intervals.remove_path(agents[id].id, agents[id].path, current_timestep, planning_period, MAX_TIMESTEP, true);
            }
