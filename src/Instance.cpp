...
int Instance::randomWalk(int curr, int steps) const
{
	...
	{
		vector<int> next_locations = getNeighbors(curr);
...
const std::vector<int> Instance::getNeighbors(int curr) const
{
	std::vector<int> neighbors_;
	neighbors_.reserve(4);

	int next;
	// Handling first candidate: curr + 1
	next = curr + 1;
	if (validMove(curr, next))
		neighbors_.emplace_back(next);

	// Handling second candidate: curr - 1
	next = curr - 1;
	if (validMove(curr, next))
		neighbors_.emplace_back(next);

	// Handling third candidate: curr + num_of_cols
	next = curr + num_of_cols;
	if (validMove(curr, next))
		neighbors_.emplace_back(next);

	// Handling fourth candidate: curr - num_of_cols
	next = curr - num_of_cols;
	if (validMove(curr, next))
		neighbors_.emplace_back(next);
...
bool Instance::validateSolution(const vector<Path *> &paths, int sum_of_costs, int num_of_colliding_pairs) const
{
	...
	if (sum_of_costs != sum)
	{
		cerr << "The computed sum of costs " << sum_of_costs << " is different from that of the solution " << sum << endl;
		exit(-1);
	}
	// check for colliions
	int collisions = 0;
		...
		for (auto j = i + 1; j < start_locations.size(); j++)
		{
			...
			for (; t < (int)paths[a1]->size(); t++)
			{
				if (paths[a1]->at(t).location == paths[a2]->at(t).location) // vertex conflict
				{
					...
					{
						cerr << "Find a vertex conflict between agents " << a1 << " and " << a2 << " at location " << paths[a1]->at(t).location << " at timestep " << t << endl;
						exit(-1);
					}
					collisions++;
						 ...
						 paths[a1]->at(t - 1).location == paths[a2]->at(t).location) // edge conflict
				{
					if (num_of_colliding_pairs == 0)
					{
						cerr << "Find an edge conflict between agents " << a1 << " and " << a2 << " at edge (" << paths[a1]->at(t - 1).location << "," << paths[a1]->at(t).location << ") at timestep " << t << endl;
						exit(-1);
					}
					collisions++;
					found_collision = true;
			...
			if (!found_collision)
			{
				...
				for (; t < (int)paths[a2]->size(); t++)
				{
					if (paths[a2]->at(t).location == target) // target conflict
					{
						if (num_of_colliding_pairs == 0)
						{
							cerr << "Find a target conflict where agent " << a2 << " (of length " << paths[a2]->size() - 1 << ") traverses agent " << a1 << " (of length " << paths[a1]->size() - 1 << ")'s target location " << target << " at timestep " << t << endl;
							exit(-1);
						}
						collisions++;
						break;
	...
	{
		cerr << "The computed number of colliding pairs " << num_of_colliding_pairs << " is different from that of the solution " << collisions << endl;
		exit(-1);
	}
	cout << "Done!" << endl;
	return true;
