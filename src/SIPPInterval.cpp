...
const vector<int> SIPPIntervals::get_intervals(int from, int interval,
  ...
  for (int i = low_index; i <= high_index; i++) {
    ...
    // Edge Conflict
    if (i - 1 >= 0 && interval + 1 < intervals_[from].size() &&
...
#ifdef DEBUG_MODE
      cout << agent_id << " splitting: " << path[t].location << " @ [" << low
           << "," << start + t + 1 << ")" << endl;
#endif
...
void SIPPIntervals::remove_path(int agent_id, Path &path, int start, int period,
  ...
  for (int t = period + 1; t < t_max; t++) {
    if (t + 1 >= t_max || path[t].location != path[t + 1].location) {
#ifdef DEBUG_MODE
      cout << agent_id << " merging: " << path[t].location << " @ [" << low
           << "," << start + t + 1 << ")" << endl;
#endif
...
void SIPPIntervals::unreserve_goal(int agent_id, int location, int timestep) {
  ...
  if (location_intervals[index].low == 0 &&
      location_intervals[index].agent_id == agent_id) {
    location_intervals[index].high = timestep;
    location_intervals.emplace_back(timestep, MAX_TIMESTEP, NO_AGENT);
    return;
  }

  if (index > 0 && location_intervals[index - 1].agent_id == NO_AGENT) {
...
void SIPPIntervals::reserve_goal(int agent_id, int location, int timestep) {
...
#ifdef DEBUG_MODE
  cout << "agent: " << agent_id << " reservation on goal location: " << location
  ...
  if (index > 0 && location_intervals[index - 1].agent_id == agent_id &&
...
void SIPPIntervals::truncate(int agent_id, int location, int timestep) {
  ...
  if (index > 0 && location_intervals[index - 1].agent_id == NO_AGENT &&
    ...
    location_intervals.erase(location_intervals.begin() + index,
                             location_intervals.begin() + index + 2);
  ...
  if (location_intervals.size() == 1 && timestep == 0) {
    location_intervals[index].agent_id = NO_AGENT;
    return;
  }
...
void SIPPIntervals::split(int agent_id, int location, int low, int high) {
  ...
  if (location_intervals[interval_index].agent_id != NO_AGENT) {
    cout << "ERROR: Agent " << agent_id << " interval already allocated to "
         << location_intervals[interval_index].agent_id << endl;
    exit(1);
  }
  ...
  if (interval_index > 0 && location_intervals[interval_index].low == low &&
      location_intervals[interval_index - 1].agent_id == agent_id &&
      location_intervals[interval_index].high -
              location_intervals[interval_index].low ==
          1) {
  ...
  if (location_intervals[interval_index].high == high) {
    ...
    location_intervals.emplace(location_intervals.begin() + interval_index + 1,
                               low, high, agent_id);
  ...
  if (location_intervals[interval_index].low == low) {
    location_intervals[interval_index].low = high;
    location_intervals.emplace(location_intervals.begin() + interval_index, low,
                               high, agent_id);
...
void SIPPIntervals::merge(int agent_id, int location, int low, int high) {
  ...
  if (index > 0 && index < location_intervals.size() - 1 &&
      location_intervals[index - 1].agent_id == NO_AGENT &&
      location_intervals[index + 1].agent_id == NO_AGENT) {
    location_intervals[index - 1].high = location_intervals[index + 1].high;
    location_intervals.erase(location_intervals.begin() + index,
...
void SIPPIntervals::validate(int location) const {
  ...
  for (int i = 0; i < location_intervals.size() - 1; i++) {
    ...
    if (location_intervals[i].high != location_intervals[i + 1].low) {
      cerr << "ERROR: interval " << i << " and " << i + 1 << " do not touch"
           << endl;
      exit(1);
    }
  }
