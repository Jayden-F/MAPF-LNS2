...
void SIPP::updatePath(const LLNode *goal, vector<PathEntry> &path,
    ...
    while (curr->parent != nullptr) // non-root node
    {
        ...
        t = prev->timestep + 1;
        while (t < curr->timestep) {
            path[t - start_time].location =
                prev->location; // wait at prev location
            t++;
        }
        path[curr->timestep - start_time].location =
            curr->location; // move to curr location
        curr = prev;
    }
    assert(curr->timestep == start_time);
    path[0].location = curr->location;
}
...
//     Path path;
//     Interval interval =
//     reservation_table.get_first_safe_interval(start_location); if
//     (get<0>(interval) > 0)
//         return path;
//     auto holding_time = constraint_table.getHoldingTime(goal_location,
//     constraint_table.length_min); auto last_target_collision_time =
//     constraint_table.getLastCollisionTimestep(goal_location);
//     // generate start and add it to the OPEN & FOCAL list
//     auto h = max(max(my_heuristic[start_location], holding_time),
//     last_target_collision_time + 1); auto start = new
//     SIPPNode(start_location, 0, h, nullptr, 0, get<1>(interval),
//     get<1>(interval),
...
//             break;
//         }
//         else if (curr->location == goal_location && // arrive at the goal
//         location
//                  !curr->wait_at_goal &&             // not wait at the goal
//                  location curr->timestep >= holding_time)    // the agent can
//                  hold the goal location afterward
//         {
//             int future_collisions =
//             constraint_table.getFutureNumOfCollisions(curr->location,
//             curr->timestep); if (future_collisions == 0)
//             {
//                 updatePath(curr, path);
//                 break;
//             }
...
//                 next_v_collision, next_e_collision) = i; if (next_timestep +
//                 my_heuristic[next_location] > constraint_table.length_max)
//                     break;
//                 auto next_collisions = curr->num_of_conflicexit(-1);ts +
//                                        // (int)curr->collision_v *
//                                        max(next_timestep - curr->timestep -
//                                        1, 0) + // wait time
//                                        (int)next_v_collision +
...
//                 auto next = new SIPPNode(next_location, next_timestep,
//                 next_h_val, curr, next_timestep,
//                                          next_high_generation,
//                                          next_high_expansion,
...
//             curr->high_expansion) && get<0>(interval) + curr->h_val <=
//             constraint_table.length_max)
//         {
//             auto next_timestep = get<0>(interval);
//             auto next_h_val = max(my_heuristic[curr->location],
//             (get<2>(interval) ? holding_time : curr->getFVal()) -
//             next_timestep); // path max auto next_collisions =
//             curr->num_of_conflicts +
//                                    // (int)curr->collision_v *
//                                    max(next_timestep - curr->timestep - 1, 0)
//                                    + (int)get<2>(interval);
//             auto next = new SIPPNode(curr->location, next_timestep,
//             next_h_val, curr, next_timestep,
...
// Returns a path that minimizes the collisions with the paths in the path
// table, breaking ties by the length
Path SIPP::findPath(pqueue_min &open, SIPPIntervals &sipp_intervals,
                    MemoryPool &memory_pool, int start_timestep,
                    int depth_limit) {
    memory_pool.reset();
    open.clear();
    reset();
    ...
    bool success = sipp_intervals.get_first_interval(
        start_location, start_timestep, start_interval);
    if (!success) {
        return path;
    }

    // int last_target_collision_time =
    // reservation_table.constraint_table.getLastCollisionTimestep(goal_location);
    // generate start and add it to the OPEN & FOCAL list
    // int h = max(max(my_heuristic[start_location], holding_time),
    // last_target_collision_time + 1);
    int h = my_heuristic[start_location];
    // auto start = new SIPPNode(start_location, 0, h, nullptr, 0,
    // get<1>(interval), get<1>(interval),
    //                             get<2>(interval), get<2>(interval));
    u_int64_t node_id = static_cast<u_int64_t>(start_location) *
                            static_cast<u_int64_t>(depth_limit) +
                        min(static_cast<u_int64_t>(depth_limit),
                            static_cast<u_int64_t>(start_interval->second.high) -
                                static_cast<u_int64_t>(start_timestep));

    SIPPNode *start = memory_pool.generate_node(
        node_id, start_location, start_timestep, h, nullptr, start_timestep,
        start_interval);
    open.push(start);
    SIPPNode next;
    ...
    while (open.size() > 0) {
        ...
        if (curr->location == goal_location &&
            curr->interval->second.high == MAX_TIMESTEP) {
        ...
        // {
        //     int future_collisions =
        //     reservation_table.constraint_table.getFutureNumOfCollisions(curr->location,
        //     curr->timestep); if (future_collisions == 0)
        //     {
        //         updatePath(curr, path);
        //         break;
        ...
        //     goal.num_of_conflicts += future_collisions;
        //     // try to retrieve it from the hash table
        //     if (dominanceCheck(curr->id, &goal, memory_pool))
        //     {
        //         *curr = goal;
        //         pushNodeToFocal(curr);
        //     }
        // }
        ...
        {
            for (iterator interval_iterator :
                 sipp_intervals.get_intervals(curr->location, curr->interval,
                                              curr->g_val + 1, next_location)) {
                // int next_high_generation, next_timestep, next_high_expansion;
                // bool next_v_collision, next_e_collision;
                // tie(next_high_generation, next_timestep, next_high_expansion,
                // next_v_collision, next_e_collision) = i; interval =
                // sipp_intervals.get_interval(next_location, interval_index);
                assert(interval_iterator->second.agent_id == NO_AGENT);
                int next_timestep =
                    max(curr->g_val + 1, interval_iterator->second.low);
                ...
                //                       // (int)curr->collision_v *
                //                       max(next_timestep - curr->timestep - 1,
                //                       0) + // wait time (int)next_v_collision
                //                       + (int)next_e_collision;
                // generate (maybe temporary) node
                // SIPPNode* next = memory_pool.generate_node( ,next_location,
                // next_timestep, next_h_val, curr, next_timestep,
                //                          next_high_generation,
                //                          next_high_expansion,
                //                          next_v_collision, next_collisions);

                next = SIPPNode(next_location, next_timestep,
                                my_heuristic[next_location], curr,
                                next_timestep, interval_iterator);
                // try to retrieve it from the hash table
                node_id = static_cast<u_int64_t>(next.location) *
                              static_cast<u_int64_t>(depth_limit) +
                          min(static_cast<u_int64_t>(depth_limit),
                              static_cast<u_int64_t>(next.interval->second.high) -
                                  static_cast<u_int64_t>(start_timestep));
            ...
            // // wait at the current location
            // if (curr->high_expansion == curr->high_generation &&
            //     reservation_table.find_safe_interval(interval,
            //     curr->location, curr->high_expansion, true) &&
            //     get<0>(interval) + curr->h_val <=
            //     reservation_table.constraint_table.length_max)
            // {
            //     int next_timestep = get<0>(interval);
            //     int next_h_val = max(my_heuristic[curr->location],
            //     (get<2>(interval) ? holding_time : curr->getFVal()) -
            //     next_timestep); // path max int next_collisions =
            //     curr->num_of_conflicts +
            //                           // (int)curr->collision_v *
            //                           max(next_timestep - curr->timestep - 1,
            //                           0) + (int)get<2>(interval);
            //     SIPPNode next(curr->location, next_timestep, next_h_val,
            //     curr, next_timestep,
            //                   get<1>(interval), get<1>(interval),
            //                   get<2>(interval), next_collisions);
            //     next.wait_at_goal = (curr->location == goal_location);
            //     int node_id = next.location * depth_limit + min(depth_limit,
            //     get<1>(interval)); if (dominanceCheck(node_id, &next,
            //     memory_pool))
            //     {
            //         SIPPNode *next_ptr = memory_pool.replace_node(node_id,
            //         next); pushNodeToFocal(next_ptr);
            //     }
            // }
        } // end while loop
...
pair<Path, int> SIPP::findSuboptimalPath(
    ...
    auto start = new SIPPNode(start_location, 0,
                              max(my_heuristic[start_location], holding_time),
    ...
    while (!open_list.empty()) {
                 ...
                 curr->location)) // move to neighboring locations
        {
            for (auto &i : reservation_table.get_safe_intervals(
                ...
                int next_h_val = max(my_heuristic[next_location],
                ...
                int next_conflicts =
                    curr->num_of_conflicts +
                    //(int)curr->collision_v * max(next_timestep -
                    // curr->timestep - 1, 0) +
...
/*Path SIPP::findNoCollisionPath(const ConstraintTable& constraint_table)
{
    ...
    Path path;
    Interval interval =
reservation_table.get_first_safe_interval(start_location); if (get<0>(interval)
> 0 or get<2>(interval)) return path; auto holding_time =
max(constraint_table.getHoldingTime(goal_location, constraint_table.length_min),
                            constraint_table.getLastCollisionTimestep(goal_location)
+ 1);
    // generate start and add it to the OPEN & FOCAL list

    auto start = new SIPPNode(start_location, 0,
max(my_heuristic[start_location], holding_time), nullptr, 0, interval, 0);
    pushNodeToFocal(start);
    while (!focal_list.empty())
    {
        auto* curr = focal_list.top();
        ...
        if (curr->location == goal_location && // arrive at the goal location
            !curr->wait_at_goal && // not wait at the goal location
            curr->timestep >= holding_time && // the agent can hold the goal
location afterward constraint_table.getFutureNumOfCollisions(curr->location,
curr->timestep) == 0) // no future collisions
        {
            updatePath(curr, path);
            break;
        }
        for (int next_location : instance.getNeighbors(curr->location)) // move
to neighboring locations
        {
            int next_h_val = my_heuristic[next_location];
            for (auto& interval : reservation_table.get_safe_intervals(
                    curr->location, next_location, curr->timestep + 1,
get<1>(curr->interval) + 1))
            {
                if (get<2>(interval.first))
                    continue;
                if (interval.second + next_h_val > constraint_table.length_max)
                    break;
                generateChildToFocal(interval.first, curr, next_location,
interval.second, next_h_val);
            }
        }  // end for loop that generates successors
        // wait at the current location
        bool found = reservation_table.find_safe_interval(interval,
curr->location, get<1>(curr->interval)); if (found and !get<2>(interval))
        {
            generateChildToFocal(interval, curr, curr->location,
get<0>(interval), curr->h_val);
        }
    }  // end while loop
...
int SIPP::getTravelTime(int start, int end,
                        const ConstraintTable &constraint_table,
                        int upper_bound) {
    reset();
    min_f_val = -1; // this disables focal lis
...
bool SIPP::dominanceCheck(pqueue_min &open, u_int64_t id, SIPPNode *new_node,
    ...
    // }
    // else if (old_node->timestep >= new_node->timestep and
    //          old_node->num_of_conflicts >= new_node->num_of_conflicts) // the
    //          old node is dominated by the new node
    // {                                                                  //
    // delete the old node
    //     if (!old_node->is_closed)                                      // the
    //     old node has not been expanded yet
    ...
    //     // ptr->second.remove(old_node);
    //     num_generated--; // this is because we later will increase
    //     num_generated when we insert the new node into lists. return true;
    // }
    // else if (old_node->timestep < new_node->interval->high and
    // new_node->timestep < old_node->interval->high) { // intervals overlap -->
    // we need to split the node to make them disjoint
    //     if (old_node->timestep <= new_node->timestep)
    //     {
    //         assert(old_node->num_of_conflicts > new_node->num_of_conflicts);
    //         old_node->high_expansion = new_node->timestep;
