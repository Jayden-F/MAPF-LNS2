...
// find path by A*
// Returns a path that minimizes the collisions with the paths in the path table, breaking ties by the length
Path SIPP::findPath(SIPPIntervals &sipp_intervals, MemoryPool &memory_pool, int start_time, int depth_limit)
{
    memory_pool.reset();
    reset();
    // Path path = findNoCollisionPath(constraint_table);
    ...
    //  ReservationTable reservation_table(constraint_table, goal_location);
    Path path;
    SIPPInterval *interval = sipp_intervals.get_first_interval(start_location, start_time)[0];
    if (interval == nullptr)
        return path;
    ...
    SIPPNode *start = memory_pool.generate_node(start_location * depth_limit + min(depth_limit, interval->high), start_location, 0, h, nullptr, 0, interval->high, interval->high);
    ...
    while (!focal_list.empty())
    {
        ...
        for (int next_location : instance.getNeighbors(curr->location)) // move to neighboring locations
        {
            for (auto &i : sipp_intervals.get_safe_intervals(
                     curr->location, next_location, curr->timestep + 1, curr->high_expansion + 1, true))
            {
                ...
                SIPPNode next(next_location, next_timestep, my_heuristic[next_location], curr, next_timestep, i->low, i->high);
                // try to retrieve it from the hash table
